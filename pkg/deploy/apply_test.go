...
func TestWithAwaitableResource(t *testing.T) {
	testCases := []struct {
	...
	}{
		{
			...
			resFileName:   "testdata/apply/simple-job.yaml",
			watcherEvents: []unstructured.Unstructured{},
			errorRequire:  require.Nil,
		}, {
			desc:        "Awaits annotated resources for completion",
			resFileName: "testdata/apply/awaitable-job.yaml",
			watcherEvents: []unstructured.Unstructured{
				{
					Object: map[string]interface{}{
						"apiVersion": "batch/v1",
	...
	for _, tC := range testCases {
		t.Run(tC.desc, func(t *testing.T) {
			...
			resources, err := NewResources(tC.resFileName, "default")
			require.Nil(t, err)
			res := resources[0]
			...
			applyCalled := false
			err = withAwaitableResource(func(c *K8sClients, r Resource, d DeployConfig) error {
				applyCalled = true
	...
	t.Run("Forwards inner apply errors", func(t *testing.T) {
		...
		actualErr := withAwaitableResource(func(clients *K8sClients, res Resource, deployConfig DeployConfig) error {
			return expectedErr
		})(&clients, res, deployConfig)
...
func TestWithDeletableResource(t *testing.T) {
	resName := "res-name"
	requireResourceExists := func(shouldExist bool) func(t *testing.T, clients *K8sClients, res Resource) {
		return func(t *testing.T, clients *K8sClients, res Resource) {
			gvr, err := FromGVKtoGVR(clients.discovery, *res.GroupVersionKind)
			require.Nil(t, err)
	...
	testCases := []struct {
		...
		setup       func(t *testing.T, clients *K8sClients, res Resource)
		requireFn   func(t *testing.T, clients *K8sClients, res Resource)
	}{
		{
	...
	for _, tC := range testCases {
		t.Run(tC.desc, func(t *testing.T) {
			...
			applyCalled := false
			err := withDeletableResource(func(clients *K8sClients, res Resource, deployConfig DeployConfig) error {
				applyCalled = true
