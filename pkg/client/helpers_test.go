...
func newTestApplier(t *testing.T, objects []*unstructured.Unstructured, generators ...generator.Interface) *Applier {
	t.Helper()

	applier, err := NewBuilder().
		WithFactory(factoryForTesting(t, objects)).
...
func factoryForTesting(t *testing.T, objects []*unstructured.Unstructured) util.ClientFactory {
	t.Helper()
